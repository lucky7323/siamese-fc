vid_path = '/home/eunho/LARGE_DATASET2/test1/';
...
for i = 1:numel(test_videos)
    ...
    runAll_new(test_videos(i).name, num_val_files, opts, model, rpn_net, fast_rcnn_net)
     if i==ceil(numel(test_videos) / 4)
         sprintf('======== 25%% complete ========= \n')
         copyfile('submission1221.txt', 'submission1221_25_cache.txt');
         pause(1.0);
     elseif i==ceil(numel(test_videos) / 2)
         sprintf('======== 50%% complete ========= \n')
         copyfile('submission1221.txt', 'submission1221_50_cache.txt');
         pause(1.0);
     elseif i==ceil(3 * numel(test_videos) / 4)
         sprintf('======== 75%% complete ========= \n')
         copyfile('submission1221.txt', 'submission1221_75_cache.txt');
         pause(1.0);
     end
end
caffe.reset_all(); 
